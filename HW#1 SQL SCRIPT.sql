...
# SQL Week 1 Assignement

# Question 1
select dest, max(air_time) from flights
group by dest
order by max(air_time) desc
limit 0,1
;
...
# Question 8
# Show the top 3 airlines traveling from JFK to SFO, by average flight time and average arrival delay, order by average arrival delay in asc order

select carrier, origin, dest, avg(air_time), avg(arr_delay) from flights
where origin = 'jfk' and dest = 'sfo'
group by carrier
order by avg(arr_delay) asc
limit 0,3
;
